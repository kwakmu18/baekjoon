    ...
    while (!q.empty()) {
        ...
        for(int i=0;i<4;i++) {
            ...
            if (newY>=0 && newY < N && newX>=0 && newX<N) {
                ...
                else if (curLab[newY][newX]==2 && virusResult[index][newY][newX]==-1) BFS(index, newY, newX, newCost);
            }
        }
    }
}
...
void DFS(int index, int curY, int curX, int curCost) {
    ...
    else if (virusResult[index][curY][curX]!=-1 && virusResult[index][curY][curX]<=curCost) return;
...
void SelectVirus(int curCnt, int curIndex) {
    if (curCnt == M) {
        ...
        for(int y=0;y<N;y++) {
            for(int x=0;x<N;x++) {
                if (curLab[y][x] == 1 || curLab[y][x] == 2) continue;
